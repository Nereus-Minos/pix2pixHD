...
from activations import ins_norm, relu, pool, tanh, lrelu
from conv_base import conv, conv_trans


def res_block(name,x, channels=2048):   # conv('conv1', x, 3*3, 64, 2, ref_pad=1, pad=True)
...
def G_base(name, x, batch):
    ...
    with tf.variable_scope(name):
        ...
        conv5 = conv('conv5', relu4, 3*3, first_out_channels * 16, 2, 0, True)
        ...
        up1 = conv_trans('up1', x_in, 3*3, first_out_channels * 8, 2, batch, True)
        ins_up1 = ins_norm('ins_up1', up1)
        relu_up1 = relu('relu_up1', ins_up1)
        
        up2 = conv_trans('up2', relu_up1, 3*3, first_out_channels * 4, 2, batch, True)
        ins_up2 = ins_norm('ins_up2', up2)
        relu_up2 = relu('relu_up2', ins_up2)
        
        up3 = conv_trans('up3', relu_up2, 3*3, first_out_channels * 2, 2, batch, True)
        ins_up3 = ins_norm('ins_up3', up3)
        relu_up3 = relu('relu_up3', ins_up3)
        
        up4 = conv_trans('up4', relu_up3, 3*3, first_out_channels, 2, batch, True)
